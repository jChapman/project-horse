<template>
  <div class="PreSelectedGod">
    <img
      class="GodComplexity"
      :src="`/images/gods/god_complexity_${god.complexity}.png`"
    />
    <div
      class="GodCardContainer"
      :class="{
        Plus: god.unlock === 'plus',
        SmallPowers: god.powers.length > 2,
      }"
    >
      <img
        class="GodCard_Avatar"
        :src="`/images/gods/images/${god.id}_avatar.png`"
      />
      ...
      <img
        class="GodCard_Universe"
        v-if="god.universe !== 'none'"
        :src="`/images/logos/${god.universe}_logo.png`"
      />
      <div class="GodCard_Info" :id="`${god.id}_info`"></div>
      ...
      <div class="GodCard_Abilities">
        <div
          v-for="power of god.powers"
          :key="power.id"
          :id="power.id"
          class="GodCard_PowerButton"
          :class="{ PassiveAbility: power.type === 'passive' }"
        >
          <div class="GodCard_AbilityBackground">
            <img
              class="GodCard_Ability"
              :src="`/images/gods/powers/power_${power.id}.png`"
            />
            <img
              class="GodCard_AbilityFrame"
              v-if="power.type !== 'passive'"
              src="./active_ability_frame.png"
            />
          </div>
          <b-tooltip
            :target="power.id"
            custom-class="PowerTooltipContainer"
            placement="bottom"
          >
            <div class="PowerTooltip GenericTooltip">
              ...
              <div class="GenericTooltip__Body">
                ...
                <div
                  v-if="power.values.length > 0 && power.values[0].length > 0"
                  class="Values"
                >
                  <div
                    v-for="value in power.values"
                    :key="value"
...
.GodCardContainer.Plus {
  filter: drop-shadow(0 3px 6px rgba(0, 204, 255, 0.5));
}
